...
2. **Load context for that task** (keep it minimal):
   ...
   - **Consult `decision-records/` for this task's subsystem:** grep `decision-records/` for records whose label matches the task's subsystem/component; pass any bearing record (accepted, open, or a rejected/anti-requirement record) by path — not contents — to the worker agent's launch prompt, so the work honors and does not silently re-litigate it.
