...
| Project-specific lesson, environment gotcha, or durable reference FACT (no code involved; pure knowledge about the codebase, environment, or external system that future-Claude needs) | → `PASSDOWN.md` "Lessons Learned / Gotchas". A neutral durable fact (not learned from a mistake) is prefixed `FACT:`; see the "Lessons Learned / Gotchas" discriminator in `step-1-concept.md`. |
| "Why we chose X" decision — durable design reasoning a future session might undo, including a deliberate omission / anti-requirement | → **Create a record in `decision-records/`** with `Status: accepted`/`rejected` (a still-undecided one is `Status: open`) — see "Project Decision Records" in `step-1-concept.md` for the filename, append-only, and supersede rules. Do NOT put it in `PASSDOWN.md` or the gitignored `decisions/` scratch folder. |
| Open question that wasn't answered this task and isn't currently blocking — a genuine unknown needing investigation, NOT a pending design decision | → `PASSDOWN.md` "Open Questions". If it's really a not-yet-made design decision a future session might re-open ("which way on X"), route to `decision-records/` with `Status: open` (the decisions row above) instead. |
...
**Governing principle — history AND decisions are append-only.** Two homes are never swept: git history and `decision-records/`. The disposition sweep only ever removes entries from `PASSDOWN.md` — it is never license to delete a decision record or rewrite history.
