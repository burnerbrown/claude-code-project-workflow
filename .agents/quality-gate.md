...
**Reject if:** No security test cases listed, the test-data policy statement (T6) is absent, tests lack host-safe vs sandbox-required classification, or integration tests exist without sandbox setup files. (Substance check of test-data contents — does it actually contain real credentials? — is owned by the Security Reviewer's pass on test code, not by QG.)
