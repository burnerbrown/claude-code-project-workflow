...
| D10 | Decision-Record Format Compliance | If any decision records are included, each follows the `step-1-concept.md` "Project Decision Records" format: an `# <id> — <title>` heading, filename `<label>-<NNN>-<slug>.md`, header fields (Date, Subsystem / label, `Status: open\|accepted\|rejected`, Supersedes), and the Context / Decision / Why / Alternatives considered sections. If no decision records are included, this criterion is N/A. |
